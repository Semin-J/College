  ...
  LVList<SummableLVPair<std::string, std::string>,std::string, std::string, 50> references;
	...
	do {
		...
		if (index) {
			index >> reference;
			index.ignore();
			// TODO: create a temporary SummableLVPair from ticketType(isn't it label type?) and number(isn't it reference?)
  ...
  LVList<SummableLVPair<std::string, int>, std::string, int, 50> ticketSales;
	...
	do {
		...
		if (sales) {
			...
			//
      SummableLVPair<std::string, int> temp(ticketType, number);
      ticketSales += temp;
		}
	} while (sales);
	sales.clear();
